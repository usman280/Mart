...
function Shop3Accounts() {
    ...
    const [shop3Accounts, setShop3Accounts] = useState([]);
    ...
    function addExpense() {
        ...
        if (expensetype === 'Manager') {
            ...
            database.ref('shop3').child("Accounts").child(monthNames[Month - 1]).child(strdate).update(amount).then(
                res => {
            ...
            if (personname === 'Hamza') {
                if (todaycompanyamount.length !== 0) {
                    ...
                    database.ref('shop3').child("Accounts").child(monthNames[Month - 1]).child(strdate).child("company").update(amount).then(
                        res => {
                            console.log("Success Account", res);
                            setExpense("");
                            setExpenseType("");
    ...
    useEffect(() => {
        ...
        const fetchAccountsData = () => {
            database.ref("shop3").child("Accounts").child(monthNames[Month - 1]).on("value", (snapshot) => {
        ...
        const fetchTodaysData = () => {
            ...
            database.ref("shop3").child("Accounts").child(monthNames[Month - 1]).child(strdate).child("company").on("value", (snapshot) => {
                if (snapshot.exists()) {
    ...
    return (
        ...
        >
            ...
            <AccountsTable
                mytitle="Shop 3 Accounts"
                mydata={shop3Accounts}
            />
