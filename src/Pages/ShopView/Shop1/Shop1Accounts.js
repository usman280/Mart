...
function Shop1Accounts() {
    ...
    const [shop1Accounts, setShop1Accounts] = useState([]);
    ...
    function addExpense() {
        ...
        if (expensetype === 'Manager') {
            ...
            database.ref('shop1').child("Accounts").child(monthNames[Month - 1]).child(strdate).update(amount).then(
                res => {
            ...
            if (personname === 'Hamza') {
                if (todaycompanyamount.length !== 0) {
                    ...
                    database.ref('shop1').child("Accounts").child(monthNames[Month - 1]).child(strdate).child("company").update(amount).then(
                        res => {
                            console.log("Success Account", res);
                            setExpense("");
                            setExpenseType("");
    ...
    useEffect(() => {
        ...
        const fetchAccountsData = () => {
            database.ref("shop1").child("Accounts").child(monthNames[Month - 1]).on("value", (snapshot) => {
        ...
        const fetchTodaysData = () => {
            ...
            database.ref("shop1").child("Accounts").child(monthNames[Month - 1]).child(strdate).child("company").on("value", (snapshot) => {
                if (snapshot.exists()) {
    ...
    return (
        ...
        >
            ...
            <AccountsTable
                mytitle="Shop 1 Accounts"
                mydata={shop1Accounts}
            />
