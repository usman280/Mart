...
function Shop2Accounts() {
    ...
    const [shop2Accounts, setShop2Accounts] = useState([]);
    ...
    function addExpense() {
        ...
        if (expensetype === 'Manager') {
            ...
            database.ref('shop2').child("Accounts").child(monthNames[Month - 1]).child(strdate).update(amount).then(
                res => {
            ...
            console.log("TYpe", todaycompanyamount);
            if (personname === 'Hamza') {
                if (todaycompanyamount.length !== 0) {
                    const amount = {
                        hamza: parseInt(expense) + parseInt(todaycompanyamount[0].companyval.hamza),
                    }
                    database.ref('shop2').child("Accounts").child(monthNames[Month - 1]).child(strdate).child("company").update(amount).then(
                        res => {
                            console.log("Success Account", res);
                            setExpense("");
                            setExpenseType("");
    ...
    useEffect(() => {
        ...
        const fetchAccountsData = () => {
            database.ref("shop2").child("Accounts").child(monthNames[Month - 1]).on("value", (snapshot) => {
        ...
        const fetchTodaysData = () => {
            ...
            database.ref("shop2").child("Accounts").child(monthNames[Month - 1]).child(strdate).child("company").on("value", (snapshot) => {
                if (snapshot.exists()) {
                    ...
                    console.log("cOmpnay..", todaycompanyamount);
                }
            })
        }
    ...
    return (
        ...
        >
            ...
            <AccountsTable
                mytitle="Shop 2 Accounts"
                mydata={shop2Accounts}
            />
